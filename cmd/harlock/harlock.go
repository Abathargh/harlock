...
If the optional filename argument is passed, it must be a valid name for an existing file, 
the contents of which will be executed. 

If a filename is passed, a number of additional args can be passed, that will be available 
within the instance of the execution. If no file is passed, the interpreter starts in 
interactive-mode.
...
Flags:`
	...
	embedUsage   = "embeds the input script into an executable " +
		"containing the interpreter runtime, instead of running the script; this requires a local go installation"
)
