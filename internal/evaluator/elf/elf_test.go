...
func TestFile_HasSection(t *testing.T) {
	...
	for _, testCase := range tests {
		if file.HasSection(testCase.name) != testCase.expected {
			t.Errorf("expectedErr HasSection(%s) = %t, got %t", testCase.name, testCase.expected, !testCase.expected)
		}
	}
}
...
func TestFile_ReadSection(t *testing.T) {
	array256 := [256]byte{}
	test2Conts := [256]byte{}
	for idx := range test2Conts {
		test2Conts[idx] = byte(idx)
	}
	...
	tests := []struct {
		...
		{".random", []byte{}, NoSuchSectionErr},
		{".random2", []byte{}, NoSuchSectionErr},
	...
	for idx, testCase := range tests {
		sectionData, err := file.ReadSection(testCase.name)
		switch testCase.expectedErr {
		case NoSuchSectionErr:
			if !errors.Is(err, testCase.expectedErr) {
				t.Errorf("expectedErr %v got %v", testCase.expectedErr, err)
			}
		case nil:
			if sectionData == nil {
				t.Errorf("unexpected nil section (cannot ever be nil)")
				continue
			}
			if len(sectionData) != len(testCase.expectedConts) ||
				!bytes.Equal(sectionData, testCase.expectedConts) {
				t.Errorf("case '%d'\nexpected: %v\ngot %v", idx, testCase.expectedConts, sectionData)
			}
		}
	}
}
...
func TestFile_WriteSection(t *testing.T) {
	...
	tests := []struct {
		...
		{".random", nil, 0, NoSuchSectionErr},
		{".random", []byte{}, 0, NoSuchSectionErr},
		{".random2", []byte{}, 0, NoSuchSectionErr},
		{".testtest", array256[:], 0, nil},
		{".testtest", array256[:], 10, OutOfBoundsErr},
		{".testtest", array300[:], 0, OutOfBoundsErr},
		{".testtest2", test2Conts[:], 0, nil},
	...
	for _, testCase := range tests {
		err := file.WriteSection(testCase.name, testCase.contents, testCase.offset)
		switch testCase.expectedErr {
		case NoSuchSectionErr:
			fallthrough
		case OutOfBoundsErr:
		...
		case nil:
			sectionData, _ := file.ReadSection(testCase.name)
			for idx, b := range testCase.contents {
				if sectionData[uint64(idx)+testCase.offset] != b {
					t.Fatalf("elf data differs from what it should be: got section[%q] = %q, "+
						"when %q was expectedErr", uint64(idx)+testCase.offset,
						sectionData[uint64(idx)+testCase.offset], b)
				}
			}
		}
	}
